...
Semua steps dan fields dibaca dari config, tidak ada hardcode.
"""
...
from  transaksional.app.config import get_form_config, settings


@dataclass
class FieldConfig:
    """Field configuration dari YAML"""
    id: str
    label: str
    step: str
    type: str
...
class DynamicFormManager:
    ...
    def config(self) -> Dict[str, Any]:
        """Get raw config dictionary - untuk akses payment_info dll"""
        return self._form_config or {}
    
    def _load_config(self):
        self._form_config = get_form_config()
        self._steps = [StepConfig.from_dict(step_data) for step_data in self._form_config.get("steps", [])]
        self._steps.sort(key=lambda s: s.order)
        for field_id, field_data in self._form_config.get("fields", {}).items():
            self._fields[field_id] = FieldConfig.from_dict(field_id, field_data)
        self._messages = self._form_config.get("messages", {})
        self._commands = self._form_config.get("commands", {})
    ...
    def get_minimum_completion(self) -> float:
        return self._form_config.get("form", {}).get("confirmation", {}).get("min_completion_percentage", 60)
    ...
    def can_confirm(self, collected_data: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        require_all = self._form_config.get("form", {}).get("confirmation", {}).get("require_all_mandatory_fields", False)
        if require_all:
